...
def vpn_script(request, location_id):
    ...
    lines = [
        ...
        "",
        "# 4. Detect ROS major version and configure VPN",
        ...
        "",
        "# ROS v7: WireGuard",
        f":if ($isV6 = false) do={{/interface wireguard add name=\"{vpn_iface}\" listen-port=13231 comment=\"SpotPay VPN\" disabled=no}}",
        f":if ($isV6 = false) do={{:if ([:len [/interface wireguard find where name=\"{vpn_iface}\"]] = 0) do={{/interface wireguard add name=\"{vpn_iface}\" listen-port=13231 comment=\"SpotPay VPN\"}}}}",
        f":if ($isV6 = false) do={{:if ([:len [/ip address find where address=\"{vpn_client_ip}/24\" interface=\"{vpn_iface}\"]] = 0) do={{/ip address add address=\"{vpn_client_ip}/24\" interface=\"{vpn_iface}\"}}}}",
        f":if ($isV6 = false) do={{:if ([:len [/interface wireguard peers find where public-key=\"{vpn_public_key}\"]] = 0) do={{/interface wireguard peers add interface=\"{vpn_iface}\" public-key=\"{vpn_public_key}\" endpoint-address=\"{vpn_server_ip}\" endpoint-port={vpn_server_port} allowed-address=\"{vpn_subnet}.0/24\" persistent-keepalive=25 comment=\"SpotPay VPS\"}}}}",
        ":if ($isV6 = false) do={:delay 2s}",
        f":if ($isV6 = false) do={{:local pubKey [/interface wireguard get \"{vpn_iface}\" public-key]; /tool fetch url=\"{register_url}\" http-method=post http-data=(\"location_id={location.id}&public_key=\" . $pubKey) keep-result=no}}",
        f":if ($isV6 = false) do={{:put (\"SpotPay setup complete (ROS v7 WireGuard). API user={api_user} VPN-IP={vpn_client_ip}\")}}",
        "",
        "# ROS v6: OpenVPN",
        f":if ($isV6 = true) do={{/tool fetch url=\"{ca_url}\" dst-path=\"spotpay-ca.pem\" mode=https}}",
        f":if ($isV6 = true) do={{/tool fetch url=\"{cert_url}\" dst-path=\"spotpay-cert.pem\" mode=https}}",
        f":if ($isV6 = true) do={{/tool fetch url=\"{key_url}\" dst-path=\"spotpay-key.pem\" mode=https}}",
        ":if ($isV6 = true) do={/certificate import file-name=spotpay-ca.pem passphrase=\"\"}",
        ":if ($isV6 = true) do={:delay 1s}",
        ":if ($isV6 = true) do={/certificate import file-name=spotpay-cert.pem passphrase=\"\"}",
        ":if ($isV6 = true) do={:delay 1s}",
        ":if ($isV6 = true) do={/certificate import file-name=spotpay-key.pem passphrase=\"\"}",
        ":if ($isV6 = true) do={:delay 1s}",
        f":if ($isV6 = true) do={{:if ([:len [/interface ovpn-client find where name=\"spotpay-vpn\"]] = 0) do={{/interface ovpn-client add name=\"spotpay-vpn\" connect-to=\"{vpn_server_ip}\" port=1194 mode=ip user=\"{api_user}\" certificate=spotpay-cert.pem_0 auth=sha1 cipher=aes256 add-default-route=no comment=\"SpotPay VPN\"}}}}",
        ":if ($isV6 = true) do={:delay 3s}",
        f":if ($isV6 = true) do={{/tool fetch url=\"{register_url}\" http-method=post http-data=\"location_id={location.id}&public_key=ovpn&vpn_ip={ovpn_client_ip}\" keep-result=no}}",
        f":if ($isV6 = true) do={{:put \"SpotPay setup complete (ROS v6 OpenVPN). API user={api_user} VPN-IP={ovpn_client_ip}\"}}",
    ]
    script = "\n".join(lines)
