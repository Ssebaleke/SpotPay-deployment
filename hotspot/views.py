...
def vpn_script(request, location_id):
    ...
    lines = [
        ...
        "",
        "# 1. API user — create if not exists, update password if exists",
        f":if ([:len [/user find name=\"{api_user}\"]] = 0) do={{/user add name=\"{api_user}\" password=\"{api_pass}\" group=full comment=\"SpotPay API User\"}}",
        f":if ([:len [/user find name=\"{api_user}\"]] > 0) do={{/user set [find name=\"{api_user}\"] password=\"{api_pass}\" group=full}}",
        "",
        "# 2. Enable API service",
        "/ip service enable api",
        "/ip service set api port=8728",
        "",
        "# 3. Firewall — allow API from VPN (skip if rule exists)",
        f":if ([:len [/ip firewall filter find comment=\"SpotPay API\"]] = 0) do={{/ip firewall filter add chain=input action=accept protocol=tcp dst-port=8728 src-address={vpn_subnet}.0/24 comment=\"SpotPay API\" place-before=0}}",
        "",
        "# 4. Detect ROS version",
        ":local rosVer [/system resource get version]",
        ":local verNum [:pick $rosVer 0 1]",
        "",
        "# 5. WireGuard setup (ROS v7 only)",
        f":if ($verNum != \"6\") do={{:if ([:len [/interface wireguard find name=\"{vpn_iface}\"]] = 0) do={{/interface wireguard add name=\"{vpn_iface}\" listen-port=13231 comment=\"SpotPay VPN\"}}}}",
        f":if ($verNum != \"6\") do={{:if ([:len [/ip address find address=\"{vpn_client_ip}/24\"]] = 0) do={{/ip address add address=\"{vpn_client_ip}/24\" interface=\"{vpn_iface}\"}}}}",
        f":if ($verNum != \"6\") do={{:if ([:len [/interface wireguard peers find comment=\"SpotPay VPS\"]] = 0) do={{/interface wireguard peers add interface=\"{vpn_iface}\" public-key=\"{vpn_public_key}\" endpoint-address=\"{vpn_server_ip}\" endpoint-port={vpn_server_port} allowed-address=\"{vpn_subnet}.0/24\" persistent-keepalive=25 comment=\"SpotPay VPS\"}}}}",
        f":if ($verNum != \"6\") do={{:delay 3s}}",
        f":if ($verNum != \"6\") do={{:local pk [/interface wireguard get [find name=\"{vpn_iface}\"] public-key]}}",
        f":if ($verNum != \"6\") do={{/tool fetch url=\"{register_url}\" http-method=post http-data=(\"location_id={location.id}&public_key=\" . [/interface wireguard get [find name=\"{vpn_iface}\"] public-key]) keep-result=no}}",
        f":if ($verNum != \"6\") do={{:put \"SpotPay WireGuard done. IP={vpn_client_ip}\"}}",
        "",
        "# 6. OpenVPN setup (ROS v6 only)",
        f":if ($verNum = \"6\") do={{/tool fetch url=\"{ca_url}\" dst-path=\"spotpay-ca.pem\" mode=https}}",
        f":if ($verNum = \"6\") do={{/tool fetch url=\"{cert_url}\" dst-path=\"spotpay-cert.pem\" mode=https}}",
        f":if ($verNum = \"6\") do={{/tool fetch url=\"{key_url}\" dst-path=\"spotpay-key.pem\" mode=https}}",
        f":if ($verNum = \"6\") do={{/certificate import file-name=spotpay-ca.pem passphrase=\"\"}}",
        f":if ($verNum = \"6\") do={{:delay 2s}}",
        f":if ($verNum = \"6\") do={{/certificate import file-name=spotpay-cert.pem passphrase=\"\"}}",
        f":if ($verNum = \"6\") do={{:delay 2s}}",
        f":if ($verNum = \"6\") do={{/certificate import file-name=spotpay-key.pem passphrase=\"\"}}",
        f":if ($verNum = \"6\") do={{:delay 2s}}",
        f":if ($verNum = \"6\") do={{:if ([:len [/interface ovpn-client find name=\"spotpay-vpn\"]] = 0) do={{/interface ovpn-client add name=\"spotpay-vpn\" connect-to=\"{vpn_server_ip}\" port=1194 mode=ip user=\"{api_user}\" certificate=spotpay-cert.pem_0 auth=sha1 cipher=aes256 add-default-route=no comment=\"SpotPay VPN\"}}}}",
        f":if ($verNum = \"6\") do={{:delay 3s}}",
        f":if ($verNum = \"6\") do={{/tool fetch url=\"{register_url}\" http-method=post http-data=\"location_id={location.id}&public_key=ovpn&vpn_ip={ovpn_client_ip}\" keep-result=no}}",
        f":if ($verNum = \"6\") do={{:put \"SpotPay OpenVPN done. IP={ovpn_client_ip}\"}}",
    ]
    script = "\n".join(lines)
