from django.urls import path
from . import views

urlpatterns = [
    path('', views.learning_page, name='learning_page'),
    path('login/', views.vendor_login, name='vendor_login'),
    path('register/', views.vendor_register, name='vendor_register'),
    path('admin-dashboard/', views.admin_dashboard, name='admin_dashboard'),
    path('admin-dashboard/vendors/<int:vendor_id>/approve/', views.admin_approve_vendor, name='admin_approve_vendor'),
    path('admin-dashboard/vendors/<int:vendor_id>/reject/', views.admin_reject_vendor, name='admin_reject_vendor'),
    path('admin-dashboard/vendors/<int:vendor_id>/suspend/', views.admin_suspend_vendor, name='admin_suspend_vendor'),
    path('admin-dashboard/vendors/<int:vendor_id>/unsuspend/', views.admin_unsuspend_vendor, name='admin_unsuspend_vendor'),
    path('admin-dashboard/withdrawals/<int:withdrawal_id>/approve/', views.admin_approve_withdrawal, name='admin_approve_withdrawal'),
    path('admin-dashboard/withdrawals/<int:withdrawal_id>/reject/', views.admin_reject_withdrawal, name='admin_reject_withdrawal'),
    path('logout/', views.vendor_logout, name='vendor_logout'),
    path('dashboard/', views.vendor_dashboard, name='vendor_dashboard'),
    path('profile/', views.vendor_profile, name='vendor_profile'),
    path('change-password/', views.vendor_change_password, name='vendor_change_password'),
    path("pay-subscription/", views.pay_subscription, name="pay_subscription"),
    path('admin-dashboard/vendors/', views.admin_vendors, name='admin_vendors'),
    path('admin-dashboard/locations-list/', views.admin_locations, name='admin_locations'),
    path('admin-dashboard/withdrawals-list/', views.admin_withdrawals, name='admin_withdrawals'),
    path('admin-dashboard/performance/', views.admin_vendor_performance, name='admin_vendor_performance'),
    path('admin-dashboard/staff/', views.admin_staff, name='admin_staff'),
    path('admin-dashboard/locations/<int:location_id>/approve/', views.admin_approve_location, name='admin_approve_location'),
    path('admin-dashboard/locations/<int:location_id>/reject/', views.admin_reject_location, name='admin_reject_location'),
    path('admin-dashboard/staff/create/', views.admin_create_staff, name='admin_create_staff'),
    path('admin-dashboard/staff/<int:user_id>/toggle/', views.admin_deactivate_staff, name='admin_deactivate_staff'),
    path('toggle-sms-notifications/', views.toggle_sms_notifications, name='toggle_sms_notifications'),
    path('transactions/', views.vendor_transactions, name='vendor_transactions'),
    path('forgot-password/', views.password_reset_request, name='password_reset_request'),
    path('reset-password/<uidb64>/<token>/', views.password_reset_confirm, name='password_reset_confirm'),
]

