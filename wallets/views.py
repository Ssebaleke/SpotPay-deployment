...
def wallet_withdraw(request):
    ...
    if request.method == 'POST':
        ...
        # Normalize payout phone to 256XXXXXXXXX
        if payout_phone:
            payout_phone = payout_phone.replace(' ', '').replace('-', '').replace('+', '')
            if payout_phone.startswith('0'):
                payout_phone = '256' + payout_phone[1:]
            elif not payout_phone.startswith('256'):
                payout_phone = '256' + payout_phone
